...
export default function ChildRegistration({ onClose, forcedError }) {
  ...
  const minDate = new Date(today.getFullYear(), today.getMonth() - 24, today.getDate()).toISOString().split("T")[0];
 ...
 const toggleAllergy = (id) => {
    ...
    const safePreferences = formData.preference_ids.filter(prefId => {
      ...
      const isBlocked = selectedCategories.some(cat => {
        ...
        const matchItem = cat.items && cat.items.some(item => 
          prefObj.name.toLowerCase().includes(item.item_name.toLowerCase()) ||
    ...
    setFormData({ ...formData, allergy_ids: newAllergies, preference_ids: safePreferences });
  };
  ...
  const handleNextStep = () => {
    ...
    if (!validateAge(formData.dob)) {
      setError("Sistem saat ini hanya berlaku untuk anak usia 6 hingga 24 bulan.");
      return;
    }
    setError("");
    setStep(2);
  ...
  const handleSubmitFinal = async () => {
    ...
    try {
      ...
      // Hit AI API Mealplan
      const insightRes = await fetch(`http://localhost:3000/api/mealplan/generate-insight/${newChildId}`, {
        method: "POST",
      ...
      if (!insightRes.ok) {
        throw new Error(insightPayload.message || "Gagal men-generate AI Insight.");
      }
  ...
  return (
    ...
    >
      ...
      <div className="flex min-h-full items-center justify-center p-4 py-12">
        ...
        {isLoading && (
          <div className="fixed inset-0 z-[400] flex flex-col items-center justify-center bg-[#8B2020] text-white">
            ...
            <h2 className="text-2xl font-bold">Menganalisis Gizi {formData.name}...</h2>
            <p className="mt-2 text-sm opacity-80">AI kami sedang menyusun rekomendasi terbaik untuk si Kecil.</p>
          </div>
        )}
      ...
      {/* ========================================== */}
      {step === 1 && !isLoading && (
        <div className="w-full max-w-[420px] rounded-[2.5rem] bg-[#8B2020] px-8 py-10 shadow-2xl text-white" onClick={e => e.stopPropagation()}>
          <h2 className="text-center text-[36px] font-bold mb-1">NutriBy</h2>
          <p className="text-center text-[15px] mb-8 opacity-90">Isi data anak anda di bawah ini</p>

          {error && <p className="mb-4 text-center text-sm font-semibold text-red-200">{error}</p>}

          <div className="space-y-6">
            <div className="flex flex-col">
              <label className="mb-1 text-sm font-semibold">Nama anak</label>
              <input type="text" name="name" value={formData.name} onChange={handleChange} className="border-b-2 border-white bg-transparent py-2 outline-none focus:border-gray-300" />
            </div>

            <div className="flex flex-col">
              <label className="mb-1 text-sm font-semibold">Tanggal lahir bayi</label>
              <input 
                type="date" 
                name="dob" 
                value={formData.dob} 
              ...
              />
              <span className="text-xs opacity-70 mt-1">Sistem kami optimal untuk usia 6 - 24 bulan</span>
            </div>

            <div className="flex flex-col">
              <label className="mb-4 text-sm font-semibold">Apa jenis kelamin anak anda</label>
              <div className="flex justify-center gap-6">
                <div 
                  onClick={() => { setFormData({ ...formData, gender: "L" }); setError(""); }}
                ...
                >
                  <div className="h-16 w-16 text-4xl flex justify-center items-center">👦🏼</div>
                  <p className={`mt-2 text-center text-xs font-bold ${formData.gender === "L" ? "text-[#8B2020]" : "text-black"}`}>Laki-laki</p>
                </div>
                <div 
                  onClick={() => { setFormData({ ...formData, gender: "P" }); setError(""); }}
                ...
                >
                  <div className="h-16 w-16 text-4xl flex justify-center items-center">👧🏻</div>
                  <p className={`mt-2 text-center text-xs font-bold ${formData.gender === "P" ? "text-[#8B2020]" : "text-black"}`}>Perempuan</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-10 flex justify-end">
            <button onClick={handleNextStep} className="font-bold text-lg hover:underline decoration-2 underline-offset-4">
              Selanjutnya
            </button>
          </div>
        </div>
      ...
      {/* ========================================== */}
      {step === 2 && !isLoading && (
        <div className="w-full max-w-[420px] rounded-[2.5rem] bg-[#8B2020] px-8 py-10 shadow-2xl text-white" onClick={e => { e.stopPropagation(); setOpenDropdown(null); }}>
          <h2 className="text-center text-[36px] font-bold mb-1">NutriBy</h2>
          <p className="text-center text-[15px] mb-8 opacity-90">Lengkapi metrik dan preferensi anak</p>

          {error && <p className="mb-4 text-center text-sm font-semibold text-red-200 bg-red-900/30 py-2 rounded">{error}</p>}

          <div className="space-y-6">
            
            <div className="flex flex-col">
              <label className="mb-1 text-sm font-semibold">Tinggi badan (dalam cm)</label>
              <input type="text" inputMode="decimal" name="tinggi" placeholder="Contoh: 70.5" value={formData.tinggi} onChange={handleNumberChange} className="border-b-2 border-white bg-transparent py-2 outline-none focus:border-gray-300 placeholder-white/50" />
            </div>

            <div className="flex flex-col">
              <label className="mb-1 text-sm font-semibold">Berat badan (dalam kg)</label>
              <input type="text" inputMode="decimal" name="berat" placeholder="Contoh: 8.2" value={formData.berat} onChange={handleNumberChange} className="border-b-2 border-white bg-transparent py-2 outline-none focus:border-gray-300 placeholder-white/50" />
            </div>

            {/* Dropdown Alergi Custom */}
            <div className="flex flex-col relative" onClick={(e) => e.stopPropagation()}>
              <label className="mb-1 text-sm font-semibold">Alergi (Bisa pilih lebih dari satu)</label>
              <div
              ...
              >
                <span className="truncate text-white/90">
                  {formData.allergy_ids.length > 0
                    ? formData.allergy_ids.map(id => formatLabel(masterAllergies.find(a => a.id === id)?.name)).join(', ')
                    : "Pilih alergi..."}
                </span>
              ...
              {openDropdown === 'allergy' && (
                <div className="absolute top-[100%] left-0 right-0 z-10 w-full max-h-40 overflow-y-auto bg-white rounded-xl mt-1 p-2 shadow-xl text-black">
                  {masterAllergies.map(a => (
                    <label key={a.id} className="flex items-center gap-3 p-2 hover:bg-gray-100 rounded cursor-pointer">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="text-sm font-medium">{formatLabel(a.name)}</span>
                    </label>
                  ))}
                </div>
              )}
            ...
            <div className="flex flex-col relative" onClick={(e) => e.stopPropagation()}>
              <label className="mb-1 text-sm font-semibold">Makanan favorit (Pilih lebih dari satu)</label>
              <div
                className="border-b-2 border-white bg-transparent py-2 outline-none cursor-pointer flex justify-between items-center"
              ...
              >
                <span className="truncate text-white/90">
                  {formData.preference_ids.length > 0
                    ? formData.preference_ids.map(id => formatLabel(masterIngredients.find(i => i.id === id)?.name)).join(', ')
              ...
              {openDropdown === 'preference' && (
                <div className="absolute top-[100%] left-0 right-0 z-10 w-full max-h-40 overflow-y-auto bg-white rounded-xl mt-1 p-2 shadow-xl text-black">
                  {masterIngredients.map(i => {
                    ...
                    const selectedCategories = masterAllergies.filter(a => formData.allergy_ids.includes(a.id));
                    ...
                    return (
                      ...
                      >
                        <input
                          ...
                          className="w-4 h-4 accent-[#8B2020]"
                        />
                        <span className={`text-sm font-medium ${isDisabled ? 'text-red-700 line-through' : 'text-black'}`}>
                          {formatLabel(i.name)}
                        </span>
                        {isDisabled && <span className="text-[10px] text-red-600 font-bold ml-auto bg-red-100 px-2 py-1 rounded-full">Alergi</span>}
                      </label>
                    );
                  })}
                </div>
            ...
            <div className="flex flex-col">
              <label className="mb-1 text-sm font-semibold">Pendapatan Orang Tua per Bulan (Rp)</label>
              <input 
                type="text" 
                inputMode="numeric" 
                name="parent_salary" 
          ...
          <div className="mt-10 flex items-center justify-between">
            <button onClick={() => setStep(1)} className="font-bold hover:underline underline-offset-4">Kembali</button>
            <button onClick={handleSubmitFinal} className="rounded-full bg-white px-8 py-2 text-[16px] font-bold text-[#8B2020] hover:bg-gray-100 transition-colors">
              Selesai
            </button>
          </div>
        </div>
      ...
      {step === 3 && !isLoading && insightData && (
        <div className="w-full max-w-[480px] rounded-[2rem] bg-white px-8 py-10 shadow-2xl text-center" onClick={e => e.stopPropagation()}>
          <h2 className="text-2xl font-bold text-gray-800">
            {formData.gender === "L" ? "Sang pangeran, " : "Sang putri, "} 
            <span className="text-[#8B2020]">{formData.name}</span> telah tiba!
          </h2>
          ...
          <div className="text-left">
            <h3 className="font-bold text-lg text-black mb-3">Informasi Umum:</h3>
            <ul className="list-disc pl-5 text-sm text-gray-700 mb-6 space-y-2">
              <li>Status Gizi Terkini: <span className="font-bold text-[#8B2020]">{childGlobalStatus}</span></li>
              <li>Saran Budget MPASI / Bulan: <span className="font-bold text-green-600">Rp {Number(insightData.max_budget_limit).toLocaleString('id-ID')}</span></li>
              <li>Estimasi Budget Aktual AI: <span className="font-bold text-blue-600">Rp {Number(insightData.actual_total_cost).toLocaleString('id-ID')}</span></li>
            </ul>

            <h3 className="font-bold text-lg text-black mb-2">Catatan dari NutriBot:</h3>
