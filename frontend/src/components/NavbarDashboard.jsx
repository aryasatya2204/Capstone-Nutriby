    ...
    <nav className="sticky top-0 z-50 w-full bg-white px-4 py-4 shadow-sm md:px-10">
      <div className="mx-auto flex max-w-7xl items-center justify-between">
        ...
        <Link to="/dashboard" className="flex items-center gap-2">
          <img src={Logo} alt="NutriBy Logo" className="h-9 w-9 object-contain" />
          <span className="text-2xl font-extrabold text-[#8B2020]">NutriBy</span>
        </Link>
        ...
        <div className="hidden items-center gap-8 md:flex">
          {navLinks.map((link) => (
            <Link
              ...
              className={`text-sm font-bold transition-colors hover:text-[#8B2020] ${
                location.pathname === link.path ? "text-[#8B2020] border-b-2 border-[#8B2020] pb-1" : "text-gray-500"
              }`}
            >
          ...
          {/* Profile Icon */}
          <Link to="/profile" className="flex h-10 w-10 items-center justify-center rounded-full bg-gray-200 text-sm font-bold text-[#8B2020] transition-transform hover:scale-105 hover:bg-gray-300">
        ...
        <div className="flex items-center gap-3 md:hidden">
          <Link to="/profile" className="flex h-9 w-9 items-center justify-center rounded-full bg-gray-200 text-xs font-bold text-[#8B2020]">
            {getInitials(user?.name)}
          </Link>
          <button 
          ...
          {navLinks.map((link) => (
            <Link
              ...
              className={`text-sm font-bold p-2 rounded-lg transition-colors ${
                location.pathname === link.path ? "bg-red-50 text-[#8B2020]" : "text-gray-600 hover:bg-gray-50"
              }`}
            >
              {link.name.toUpperCase()}
            </Link>
