...
const { 
  getAgeInMonths, calculateLMS, calculateDynamicBudget, calculateWaterlowCalories,
  getStatusWFA, getStatusHFA, getStatusWFH, getGlobalStatus
...
const createChild = async (req, res) => {
  try {
    const { name, dob, gender, parent_salary, tinggi, berat, allergy_ids, preference_ids } = req.body;
    ...
    if (allergy_ids && allergy_ids.length > 0 && preference_ids && preference_ids.length > 0) {
      ...
      for (const pref of preferences) {
        ...
        if (clash) {
          return res.status(400).json({ message: "Validasi Gagal: Data kontradiktif!", details: `Anak alergi terhadap kategori '${clash.name}', sehingga bahan '${pref.name}' dilarang menjadi makanan kesukaan.` });
        }
      }
    }
    ...
    const [whoWFA, whoHFA, whoWFH, akgData] = await Promise.all([
      ...
      prisma.akgStandard.findFirst({ where: { gender, min_age_months: { lte: ageInMonths }, max_age_months: { gte: ageInMonths } } })
    ]);

    if (!whoWFA || !whoHFA || !whoWFH || !akgData) return res.status(404).json({ message: "Gagal mengkalkulasi. Data Standar belum tersedia." });
    ...
    const targetProtein = (parseFloat(akgData.protein) / akgBaseWeight) * idealWeight;
    const targetLemak = (parseFloat(akgData.fat) / akgBaseWeight) * idealWeight;

    const result = await prisma.$transaction(async (tx) => {
      return await tx.child.create({
        data: {
          user_id: userId, name, dob: new Date(dob), gender, parent_salary: parseFloat(parent_salary), optimal_budget_cache: budgetInfo.optimal_budget,
          ...
          growth_logs: {
            create: { record_date: new Date(), tinggi: tinggiAktual, berat: beratAktual, bbi_kg: idealWeight, zscore_wfa: zscoreWFA, status_wfa: statusWFA, zscore_hfa: zscoreHFA, status_hfa: statusHFA, zscore_wfh: zscoreWFH, status_wfh: statusWFH, global_status: globalStatus, target_kalori: targetKalori, target_protein: targetProtein, target_lemak: targetLemak, target_zinc: parseFloat(akgData.zinc), target_besi: parseFloat(akgData.iron) }
          }
        },
        include: { allergies: { include: { allergy_category: true } }, preferences: { include: { ingredient: true } }, growth_logs: true }
      });
    });

    res.status(201).json({ message: "Data anak beserta status pertumbuhan disimpan!", calculations: { persentase_gaji: `${budgetInfo.persentase_gaji}%`, budget_bulanan_rekomendasi: Math.round(budgetInfo.optimal_budget) }, data: result });
  } catch (error) { res.status(500).json({ message: "Gagal menambah data anak", error: error.message }); }
};
...
const getChildren = async (req, res) => {
  try {
    const children = await prisma.child.findMany({
      ...
      include: { 
        ...
        meal_plans: { orderBy: { id: 'desc' }, include: { items: { include: { recipe: true } } } } // Memastikan relasi ditarik
      }
    });

    // LAZY DELETION 7 Hari
    const now = new Date();
  ...
  } catch (error) { res.status(500).json({ message: "Gagal mengambil data anak", error: error.message }); }
};
  ...
  try {
    ...
    if (allergy_ids && allergy_ids.length > 0 && preference_ids && preference_ids.length > 0) {
      ...
      for (const pref of preferences) {
        const prefNameLower = pref.name.toLowerCase();
        const clash = allergies.find(al => prefNameLower.includes(al.name.toLowerCase()));
        if (clash) return res.status(400).json({ message: "Validasi Gagal: Data kontradiktif!", details: `Anak alergi terhadap '${clash.name}', '${pref.name}' dilarang.` });
      }
    }

    const existingChild = await prisma.child.findUnique({ where: { id }, include: { growth_logs: { orderBy: { record_date: 'desc' }, take: 1 } } });
    ...
    if (parent_salary !== undefined) {
      ...
      const newBudgetInfo = calculateDynamicBudget(latestLog ? parseFloat(latestLog.zscore_wfh) : 0, finalSalary);
    ...
    let updatedChild = await prisma.$transaction(async (tx) => {
      ...
      return await tx.child.update({
        where: { id },
        data: {
          parent_salary: finalSalary, optimal_budget_cache: finalBudget,
        ...
        include: { allergies: { include: { allergy_category: true } }, preferences: { include: { ingredient: true } }, growth_logs: { orderBy: { record_date: 'desc' }, take: 1 }, meal_plans: { orderBy: { id: 'desc' } } }
      });
    });
    ...
    if (parent_salary !== undefined) {
      ...
      } catch (aiError) { console.error("Gagal update AI Mealplan:", aiError.message); }
    }

    res.status(200).json({ message: "Data berhasil diperbarui!", data: updatedChild });
  } catch (error) { res.status(500).json({ message: "Gagal memperbarui data", error: error.message }); }
};
