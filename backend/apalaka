...
// Fungsi bantuan untuk mengubah format desimal (3,5 -> 3.5)
function parseIndonesianFloat(value) {
  if (!value) return 0;
  if (typeof value === 'number') return value;
  const sanitized = value.toString().replace(',', '.').replace(/[^0-9.-]+/g, "");
  return parseFloat(sanitized) || 0;
}

async function main() {
  console.log('Memulai proses pemetaan alergen, bahan utama, dan import data...');
    ...
    .on('end', async () => {
      ...
      try {
        for (const row of resultsCSV) {
          ...
          // ==========================================
          // 1. LOGIKA SCAN ALERGI
          // ==========================================
          const textToScan = `${row['name'] || ''} ${row['description'] || ''} ${row['Bahan_Masakan'] || ''} ${row['Bahan_Utama'] || ''} ${row['Potensi_Alergi'] || ''}`.toLowerCase();
          ...
          // ==========================================
          // 2. LOGIKA BAHAN UTAMA (PIVOT INGREDIENT)
          // ==========================================
          const bahanUtamaRaw = row['Bahan_Utama'] || "";
          const bahanUtamaArray = bahanUtamaRaw.split(',')
                                               .map(b => b.trim())
                                               .filter(b => b !== "");
          ...
          // ==========================================
          // 3. INSERT KE DATABASE WITH NEW COLUMN
          // ==========================================
        ...
        console.log('✅ Berhasil! Semua data resep dan kolom baru "bahan_masakan" telah terisi.');
